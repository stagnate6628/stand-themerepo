-- Pulsive by aaronlink127#0127
menu.action(menu.my_root(), "Load Recommended Settings", {},
    "If this is your first time using, you need to run this for the best experience.", function()
        menu.trigger_commands(
            "blur 0;borderwidth 4;borderrounded on;tabswidth 56;tabsposition top;tabsalignment centre;tabslefticon on;tabsname off;tabsrighticon off;menuwidth 392;primary 00000000;commandinfotextposition bottom;spacer 4;border 191919;background 191919")
...
menu.rainbow(menu.colour(menu.my_root(), "Primary Colour", {"primarycol"}, "", clr, true, function(val)
    clr = val
end))
...
local timer = time()
local pulsel = true
menu.toggle(menu.my_root(), "Reset Animation on Selection Change", {}, "", function(st)
    pulsel = st
end, pulsel)
local animsp = 0.001
menu.slider(menu.my_root(), "Animation Speed", {"animationspeed"}, "", -2147483648, 2147483647, 100, 20, function(val)
    timer = time()
    animsp = val / 100000
end)
local sep_height = 2 / 1080
menu.slider(menu.my_root(), "Seperator Height", {"seperatorheight"}, "", 0, 16, 2, 1, function(val)
    sep_height = val / 1080
end)
local last_slot
...
util.create_tick_handler(function()
    ...
    for i = 0, menu_w_pix - 1 do
        ...
        directx.draw_rect(i / 1920 + menu_x, menu_y, 1 / 1920, math.min(menu_h, 1), bg_clr.r * (1 - pct) + clr.r * pct,
            bg_clr.g * (1 - pct) + clr.g * pct, bg_clr.b * (1 - pct) + clr.b * pct, 1)
    end
    ...
    if menu.are_tabs_visible() then
        ...
        if tab_pos == 1 then -- top
            ...
            for i = 0, tab_cnt - 1 do
                ...
                directx.draw_rect(menu_x + tab_wid * i + tab_wid / 2 - 8 / 1920, menu_y - spacer_y - 3 / 1080,
                    16 / 1920, 3 / 1080, clr)
            end
    ...
    if menu.get_value(addr_ref) then
        ...
        for i = 0, addr_h_pix - 1 do
            ...
            pct = pct * 0.8 + 0.2
            pct = pct * clr.a * 0.7
            directx.draw_rect(addr_x, addr_y + i / 1080, addr_w, 1 / 1080, bg_clr.r * (1 - pct) + clr.r * pct,
                bg_clr.g * (1 - pct) + clr.g * pct, bg_clr.b * (1 - pct) + clr.b * pct, 1)
        end
    end
    return true
