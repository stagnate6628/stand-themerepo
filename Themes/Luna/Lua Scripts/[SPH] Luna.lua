...
local header_path = filesystem.resources_dir() .. 'ProfileHelper\\Themes\\Luna\\Header.bmp'
local subheader_path = filesystem.resources_dir() .. 'ProfileHelper\\Themes\\Luna\\Subheader.bmp'
local footer_path = filesystem.resources_dir() .. 'ProfileHelper\\Themes\\Luna\\Footer.bmp'

if not io.exists(header_path) then
		util.toast('[SPH] Header not found, attempting download.')
		lib:download_file('Themes/Luna/Header.bmp', {header_path})
end

if not io.exists(footer_path) then
		util.toast('[SPH] Footer not found, attempting download.')
		lib:download_file('Themes/Luna/Footer.bmp', {footer_path})
end

if not io.exists(subheader_path) then
		util.toast('[SPH] Footer not found, attempting download.')
		lib:download_file('Themes/Luna/Subheader.bmp', {subheader_path})
end
