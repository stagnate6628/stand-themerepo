...
util.create_tick_handler(function()
	if menu.is_open() then
		...
		local curr = menu.get_current_menu_list()
		local cursor_text = menu.get_active_list_cursor_text(true, false)
		...
		directx.draw_text(
			x + 0.003, y - 0.025,
			lang.get_string(curr.menu_name):upper(),
			ALIGN_TOP_LEFT,
			0.6,
			1,
			1,
		...
		directx.draw_text(
			x + 0.235, y - 0.025,
			table.concat(cursor_text:split('/'), ' / '),
			ALIGN_TOP_RIGHT,
			0.6,
			1,
			1,
