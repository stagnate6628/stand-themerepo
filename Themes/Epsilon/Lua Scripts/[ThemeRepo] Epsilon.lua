local dir = filesystem.resources_dir()
local header_path = dir .. 'ThemeRepo\\Themes\\Epsilon\\Header.bmp'
local footer_path = dir .. 'ThemeRepo\\Themes\\Epsilon\\Footer.bmp'
local subheader_path = dir .. 'ThemeRepo\\Themes\\Epsilon\\Subheader.bmp'

local function get_header_path(i)
	return dir .. $'ThemeRepo\\Themes\\Epsilon\\Interaction Header\\Header{i}.bmp'
end
...
local globe = directx.create_texture(get_header_path(1))

util.create_tick_handler(function()
	if menu.is_open() then
		for i = 1, 18 do
			util.yield(50)
			globe = directx.create_texture(get_header_path(i))
		end
		util.yield(8 * 1000)
	end
	return true
