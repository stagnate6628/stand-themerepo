...
if not io.exists(header_path) then
    ...
    downloader:download_file("Themes/Fragment/Header.bmp", {header_path})
...
if not io.exists(footer_path) then
    ...
    downloader:download_file("Themes/Fragment/Footer.bmp", {footer_path})
    util.toast("[SPH] Restarting")
    util.restart_script()
end
...
if not io.exists(subheader_path) then
    ...
    util.toast("[SPH] Footer not found, attempting download. The script will automatically restart when finished.")
    downloader:download_file("Themes/Fragment/Subheader.bmp", {subheader_path})
    util.toast("[SPH] Restarting")
    util.restart_script()
end
